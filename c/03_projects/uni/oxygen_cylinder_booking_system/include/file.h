...
static void save_shops_csv(const ShopList *shops)
{
    ...
    if (!f) { perror("open shops.csv for write"); exit(EXIT_FAILURE); }
...
static void save_cyls_csv(const CylList *cyls)
{
    ...
    if (!f) { perror("open inventory.csv for write"); exit(EXIT_FAILURE); }
...
static void save_cust_csv(const CustList *custs)
{
    ...
    if (!f){ perror("open customers.csv"); exit(EXIT_FAILURE); }
...
static void load_shops_csv(ShopList *shops)
{
    ...
    if (!f) return;                        /* first run – file not there */
    ...
    while (fgets(line, sizeof line, f)) {
                   ...
                   &s.id, name, user, pass) == 4)
        {
            ...
            shops->data = xmalloc((shops->len + 1) * sizeof(Shop));
...
static void load_cyls_csv(CylList *cyls)
{
    ...
    while (fgets(line,sizeof line,f)){
        ...
        {
            cyls->data = xmalloc((cyls->len+1)*sizeof(Cylinder));
...
static void load_cust_csv(CustList *custs)
{
    ...
    while (fgets(line,sizeof line,f)){
        ...
        if (sscanf(line,
           ...
           c.password) == 6) /* %19  */
        {
            custs->data = xmalloc((custs->len+1)*sizeof(Customer));
